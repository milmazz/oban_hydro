defmodule Hydro.CLITest do
  ...
  test "group workers by the given queues" do
    assert capture_io(fn ->
             CLI.main(~w|workers_by_queues hydro -q email_delivery|)
           end) == ~s|"email_delivery":\n\tHydro.EmailDeliver\n|
  end

  test "find and group workers by custom unique periods" do
    assert capture_io(fn ->
             CLI.main(~w|unique_workers_with_custom_period hydro|)
           end) == ~s|60:\n\tHydro.EmailDeliver\n|
  end

  test "find unique workers without the `keys` option" do
    assert capture_io(fn ->
             CLI.main(~w|unique_workers_without_keys_option hydro|)
           end) == ~s|Hydro.EmailDeliver\n|
  end

  test "find and group workers by unique group states" do
    assert capture_io(fn ->
             CLI.main(~w|workers_by_unique_state_groups hydro|)
           end) == ~s|[:available, :executing, :scheduled]:\n\tHydro.EmailDeliver\n|
  end

  test "find workers without wrapper functions" do
    assert capture_io(fn ->
             CLI.main(~w|workers_without_wrappers hydro --prefix schedule|)
           end) == ~s|Hydro.EmailDeliver\n|

    assert capture_io(fn ->
             CLI.main(~w|workers_without_wrappers hydro|)
           end) == ~s|Hydro.EmailDeliver\n|
  end
  ...
  test "prints usage when option queues is empty" do
    assert capture_io(fn ->
             CLI.main(~w|workers_by_queues hydro|)
           end) =~ ~s|Usage:\n  hydro command app [OPTIONS]\n|
  end

  test "prints usage when number of arguments is different than two" do
    assert capture_io(fn ->
             CLI.main(~w|workers_without_wrappers hydro foo|)
           end) =~ ~s|Usage:\n  hydro command app [OPTIONS]\n|
  ...
  test "raises when the given app is not loaded" do
    assert_raise(ArgumentError, ~s|cannot find the application "bar"|, fn ->
      CLI.main(~w|workers_without_wrappers bar|)
    end)
  end
end
